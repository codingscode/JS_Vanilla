// valueOf() - boolean.valueOf()    sem parametros
/*
The valueOf() method returns the primitive value of a boolean.

Note: This method is usually called automatically by JavaScript behind the scenes, and not explicitly in code.
*/

var booleano = false
var mostrar = booleano.valueOf()

console.log(`${mostrar} || seu tipo : ${typeof mostrar}`)
console.log(typ// valueOf() - boolean.valueOf()    sem parametros
/*
The valueOf() method returns the primitive value of a boolean.

Note: This method is usually called automatically by JavaScript behind the scenes, and not explicitly in code.
*/

var booleano = false
var mostrar = booleano.valueOf()

console.log(`${mostrar} || seu tipo : ${typeof mostrar}`)
console.log(typeof(booleano))


